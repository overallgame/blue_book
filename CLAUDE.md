...
- 示例：`添加评论模块ui代码，优化视频播放页布局`
- 格式：`<做了什么>，<还做了什么>`

## 架构概览

Android 应用，最低支持 API 31（Android 12），使用 **Kotlin 1.9.24**、**AGP 8.5.2**、**Java 17**。采用 **Clean Architecture** + MVVM + 自定义 **UDF（单向数据流）** 模式。所有 UI 文案、注释、API 返回信息均为中文。Git 用户：`overfloatGame`。

### 分层结构

```
presentation/   → Fragment + ViewModel（UDF: Intent → State → Effect）
domain/         → UseCase + Repository 接口 + 领域模型（纯 Kotlin，不依赖 Android）
data/           → Repository 实现、RemoteDataSource（Retrofit）、LocalDataSource（Room + SharedPreferences）、DTO、Mapper
core/           → 跨层基础设施：网络鉴权、Media3 播放器引擎、UDF 基类
di/             → Hilt @Module 定义（全部 @InstallIn(SingletonComponent)）
common/         → 公共 bean：ApiResponse<T>、VideoCardInfo
```
...
- **Base URL**：`http://10.0.2.2:8085/`（Android 模拟器本地地址），定义在 `NetworkModule`
- **响应信封**：
...
- **API 调用包装器**（`data/remote/RemoteCall.kt`）：三个 `suspend inline` 函数封装 Retrofit 调用：
...
**TokenAuthenticator**（`core/network/TokenAuthenticator.kt`）：
...
所有 Module 均为 `@InstallIn(SingletonComponent::class)`：

| Module | 提供内容 |
|--------|---------|
| `NetworkModule` | OkHttpClient（2 个）、Retrofit（`@Named("backend")`）、Gson、6 个 Retrofit API 接口 |
| `DatabaseModule` | Room `AppDatabase`、`UserDao` |
| `PreferenceModule` | `@Binds AuthPreferencesImpl → AuthPreferences`（SharedPreferences） |
| `LocalModule` | `@Binds UserLocalDataResourceImpl → UserLocalDataResource` |
| `RepositoryModule` | `@Binds` 3 个 Repository 实现 → 对应 domain 接口 |

## 导航

单 Activity（`MainActivity`）+ Jetpack Navigation（`nav_graph.xml`）：
...
大部分页面使用 **View 体系**（XML + ViewBinding）。Compose 可用（Compose BOM 2024.06.00、Material3），但当前仅用于主题定义（`ui/theme/`）。Fragment 使用 `Fragment(R.layout.xxx)` 或手动 ViewBinding 方式加载布局。
