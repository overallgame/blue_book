...
> ⚠️ **历史快照（2026-08-01 时点），不要当作现状读。** 本文的模块清单、文件数、架构评估与
> 已知 Bug 都已过时（当时 11 个模块 / 163 个 Kotlin 文件；现在是 12 个 Android 模块，且中间经过
> 三轮审计与两轮重构）。**现状看仓库根的 `CLAUDE.md`；缺陷记录看
> `代码审计报告-2026-09-11.md` 与 `第二轮审计报告-2026-09-11.md`。**
>
> **仍有效的只有第 4 节 PRD**（产品定位 / 核心功能 / 非功能性需求）——它是本仓库唯一一份产品
> 需求描述，代码本身表达不出"打算做成什么"。其余章节保留只为追溯当时的判断。
